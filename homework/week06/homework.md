...
**【工程自由度】**：我们在 `models.py` 模板中为你提供了“函数式（过程式）”和“面向对象（Class）”两种代码框架的参考。你可以自由选择实现方式，但我们**强烈建议你尝试 Class 封装**，它将极大减轻你在后续任务中的痛苦。
...
- **场景 B（真实数据与多实例）**：读取 `data/q3_marketing.csv`。假设该数据包含了“北美市场（Region A）”和“欧洲市场（Region B）”的独立特征。
  - 请分别为北美和欧洲市场**各自建立独立的模型实例**。
  - 分别对两个市场执行联合 F 检验，探究各自的广告投放策略是否有效。
...
- **硬核规定**：在展示前，请当着全班的面，将你目录下的 `results/` 文件夹彻底删除。然后在终端敲下运行命令，让大家看着程序在一秒钟内重建文件夹并吐出完整的业务报告！
- **汇报内容 (3-5 mins)**：
