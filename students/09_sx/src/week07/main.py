...
尽管曲线形状相同，两种策略都成功收敛：
- Full Batch 最终 MSE: {model_full.loss_history_[-1]:.6f}
- Mini-Batch 最终 MSE: {model_mini.loss_history_[-1]:.6f}

与解析解OLS在测试集上的表现一致（R²均约为0.89），证明梯度下降实现正确。
